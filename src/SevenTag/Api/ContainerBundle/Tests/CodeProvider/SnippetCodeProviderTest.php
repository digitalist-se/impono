...
class SnippetCodeProviderTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function itShouldGenerateCode()
    {
        ...
        $codeProvider = new SnippetCodeProvider($this->getRouterMock(), $this->getEngineMock());
    ...
    private function getRouterMock()
    {
        ...
        $mock
            ->generate('get_container_javascript', Argument::withKey('id'), UrlGeneratorInterface::NETWORK_PATH)
            ->shouldBeCalledTimes(1);
        $mock
            ->generate('get_no_script', Argument::withKey('id'), UrlGeneratorInterface::NETWORK_PATH)
            ->shouldBeCalledTimes(1);
    ...
    private function getEngineMock()
    {
        ...
        $mock
            ->render('@SevenTagContainerBundle/Resources/views/snippet.html.twig', Argument::size(3))
            ->shouldBeCalledTimes(1)
            ->willReturn('<script>
var i = 0;
var asd = null;
